...
import { Check, Loader2, Search, Upload, Play, Plus } from 'lucide-react';
...
export function SubmissionsPage() {
    ...
    const filteredGradings = gradings
        .filter(g => {
            const matchesSearch = searchTerm === '' ||
                ...
                String(g.uid || g.id).includes(searchTerm);
        ...
        .sort((a, b) => {
            ...
            if (sortBy === 'az') return (a.card_name || '').localeCompare(b.card_name || '');
            if (sortBy === 'za') return (b.card_name || '').localeCompare(a.card_name || '');
            return 0;
        });
    ...
    return (
        <div className="min-h-screen bg-[#0a0a0b] text-white font-sans selection:bg-purple-500/30">
                ...
                <div className="mb-12">
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8">
                        <div className="flex items-center gap-3 w-full md:w-auto">
                            ...
                            <h2 className="text-xl font-semibold text-gray-200 tracking-tight whitespace-nowrap">Active Submissions</h2>
                            <div className="h-px bg-gradient-to-l from-gray-700 to-transparent w-8 md:hidden" />
                        </div>

                        <div className="flex flex-wrap items-center gap-4 w-full md:w-auto">
                            {/* Status Filter */}
                            <select
                                className="bg-gray-900/50 border border-white/10 rounded-lg px-4 py-2 text-sm text-gray-300 focus:outline-none focus:border-purple-500/50 appearance-none cursor-pointer"
                                value={statusFilter}
                            ...
                            >
                                ...
                                <option value="Rejected - Counterfeit">Rejected</option>
                            </select>
            ...
            {/* Floating Action Button (Submit Card) */}
            <button
                onClick={() => setIsModalOpen(true)}
                className="fixed bottom-10 right-10 group flex items-center justify-center px-6 py-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-full font-bold hover:shadow-[0_0_30px_rgba(168,85,247,0.6)] hover:-translate-y-1 transition-all duration-300 shadow-xl shadow-purple-900/30 z-[100] overflow-hidden"
            >
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000" />
                <Plus size={24} className="mr-2 group-hover:rotate-90 transition-transform duration-300" />
                <span className="text-lg tracking-wide">Submit Card</span>
            </button>
        </div>
    );
}
